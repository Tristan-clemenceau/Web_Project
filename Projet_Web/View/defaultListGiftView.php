	...
	<div id="content">
		...
		<div id="dynamic_content">
			...
			<div id="midle_box">
				...
				<ul class="list_normal">
						<?php
						while ($row = mysqli_fetch_assoc($listGift)) {
						 	echo '<li>'.$row['nomCadeau'].'<a href="index.php?action=deleteListGift&idList='.$row['id_Liste'].'&idGift='.$row['id_Cadeau'].'"><img src="Public/Icon/cross.png" width="15px" height="15px" style="margin-left: 5px;"></a></li>';
						 } 
						 	echo '<li><a href="index.php?action=viewAddGiftLIst&id='.$idList.'"><span>Ajouter un cadeau</span></a></li>';
					?>
				</ul>
			</div>
			<div id="right_box">
				<h1>Informations</h1>
				<p><span>Partagé dans : </span> <?php while($row = mysqli_fetch_assoc($infoList)){echo '<br/>'.$row['nomGroupe'];}?></p>
			</div>
		</div>
	</div>
	<div id="mid_box">
